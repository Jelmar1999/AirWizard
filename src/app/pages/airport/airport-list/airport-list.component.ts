...
getCollectionSize(){
  this.airportService.getAirports(this.currentUser!).pipe(
    map((airports: Airport[]) => airports.length)).subscribe((size) => {this.collectionSize = size})
}
  ...
  this.airportService
    ...
    .pipe(
      map((airports: Airport[]) =>
        airports.slice((this.page - 1) * this.pageSize, (this.page - 1) * this.pageSize + this.pageSize),
      )
    )
